...
Tune with `--nodes` (calibrated to the shipped budget) so common random numbers actually work and every core is usable; gate with `bench.py` on the wall clock, one process at a time, because the clock is what ships. `bench.py` interleaves candidate and baseline per seed so ambient load drift lands on both, and takes `--bin` / `--bin-base` for cross-binary A/B. `tools/duel.py` plays paired head-to-head matches (roles and seeds mirrored, so seat and seed luck cancel) and reports win rate, Elo and LOS — the versus record is the real metric; solo attack per piece is the screening proxy.

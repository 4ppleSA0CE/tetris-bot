...
static void test_search_incoming_cancel() {
    ...
    // Six lines incoming: a non-clearing root is charged incomingRisk times at least
    // (height + 6 - 12)^2 = 900; the quad cancels four and ducks under the threshold,
    // costing only its -200 reward.
...
// Continuous playback: every fall is animated cell by cell over several frames --
// no single-frame teleport -- and every piece still comes to rest on its ghost
// before the lock. Soft drops survive only when something follows them (a tuck or
// a spin).
static void test_bot_instance_hard_drops() {
    ...
    assert(maxFrameDrop <= 12);           // and none is a full-height teleport
}
